...
from exp_mel.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_mel.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, MultiResolutionDiscriminator
from exp_mel.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def train(rank, world_size, h):
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            latent, _, commitment_loss, codebook_loss = encoder(y_mel)
            y_g = decoder(latent)
            ...
            #torch.nn.utils.clip_grad_norm_(itertools.chain(mpd.parameters(), mrd.parameters()), 0.1)
            ...
            L_G.backward()
            #torch.nn.utils.clip_grad_norm_(itertools.chain(encoder.parameters(), decoder.parameters()), 0.1)
            optim_g.step()
            ...
            if rank == 0:
                ...
                if steps % h.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        print(
                            "Steps : {:d}, Gen Loss Total : {:4.3f}, Amplitude Loss : {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Phase Time Difference Loss : {:4.3f}, Real Part Loss : {:4.3f}, Imaginary Part Loss : {:4.3f}, Mel Spectrogram Loss : {:4.3f}, Mel Spectrogram L2 Loss : {:4.3f}, Commit Loss : {:4.3f}, s/b : {:4.3f}".format(
                                steps,
                                L_G,
                                A_error,
                                IP_error,
                ...
                if steps % h.validation_interval == 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            y_mel = mel_spectrogram(audio.squeeze(1), h.n_fft, h.num_mels_for_loss, h.sampling_rate, h.hop_size, h.win_size, 0, None,)
                            logamp, pha, rea, imag = amp_pha_specturm(audio.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                            latent, _, _, _ = encoder(y_mel.to(device))
                            y_g = decoder(latent)
                            logamp_g, pha_g, rea_g, imag_g = amp_pha_specturm(y_g.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                            y_g_mel = mel_spectrogram(y_g.squeeze(1), h.n_fft, h.num_mels_for_loss, h.sampling_rate, h.hop_size, h.win_size, 0, None,)
