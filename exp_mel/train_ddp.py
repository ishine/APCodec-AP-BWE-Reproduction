...
from exp_mel.dataset import Dataset, amp_pha_specturm, get_dataset_filelist, mel_spectrogram
from exp_mel.models import (
    Decoder,
    Encoder,
    ...
    try:
        ...
        for epoch in range(max(0, last_epoch), h.training_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                y_mel = mel_spectrogram(audio.squeeze(1), h.n_fft, h.num_mels_for_loss ,h.sampling_rate, h.hop_size, h.win_size, 0, None,)

                latent, commitment_loss, codebook_loss, ctc_loss = encoder(y_mel, text_labels, target_lengths)
                
                y_g = decoder(latent)
                print(f"audio形状{audio.shape}")
                print(f"y_g形状{y_g.shape}")
                logamp_g, pha_g, rea_g, imag_g = amp_pha_specturm(y_g.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                ...
                for name, param in encoder.named_parameters():
                    if torch.isnan(param).any():
                        print(f"NaN detected in encoder param: {name}")
                optim_g.step()

                for name, param in decoder.named_parameters():
                    if torch.isnan(param).any():
                        print(f"NaN detected in decoder param: {name}")
                optim_g.step()
                ...
                if steps % h.stdout_interval == 0 and rank == 0:
                    ...
                    print(
                        "Steps : {:d}, Gen Loss Total : {:4.3f}, Amplitude Loss : {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Phase Time Difference Loss : {:4.3f}, Real Part Loss : {:4.3f}, Imaginary Part Loss : {:4.3f}, Mel Spectrogram Loss : {:4.3f}, Mel Spectrogram L2 Loss : {:4.3f}, Commit Loss : {:4.3f}, CTC Loss : {:4.3f}, s/b : {:4.3f}".format(
                            steps,
                            L_G,
                            A_error,
                            IP_error,
                ...
                if steps % h.summary_interval == 0 and rank == 0:
                    ...
                    sw.add_scalar("Training/CTC_Loss", ctc_loss, steps)
                ...
                if steps % h.validation_interval == 0 and rank == 0:  # 只在主进程进行验证
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            logamp, pha, rea, imag = amp_pha_specturm(audio.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                            y_mel = mel_spectrogram(audio.squeeze(1), h.n_fft, h.num_mels_for_loss, h.sampling_rate, h.hop_size, h.win_size, 0, None,)
                            latent, _, _, ctc_loss = encoder(y_mel.to(device), text_labels.to(device), target_lengths.to(device))
                            y_g = decoder(latent)
                            y_g_mel = mel_spectrogram(y_g.squeeze(1), h.n_fft, h.num_mels_for_loss, h.sampling_rate, h.hop_size, h.win_size, 0, None,)
                            logamp_g, pha_g, rea_g, imag_g = amp_pha_specturm(y_g.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                            ...
                            if j <= 4:
                                if steps == 0:
                                    sw.add_audio(
                                        "gt/y_{}".format(j), audio[0], steps, h.sampling_rate
                                    )
                                    sw.add_figure(
                                        "gt/y_logamp_{}".format(j),
                                        plot_spectrogram(logamp[0].cpu().numpy()),
                                        steps,
                                    )
                                    sw.add_figure(
                                        "gt/y_pha_{}".format(j),
                                        plot_spectrogram(pha[0].cpu().numpy()),
                                        steps,
                                    )
                                ...
                                sw.add_figure(
                                    ...
                                    plot_spectrogram(logamp_g[0].cpu().numpy()),
                                    steps,
                                )
                                sw.add_figure(
                                    "generated/y_g_pha_{}".format(j),
                                    plot_spectrogram(pha_g[0].cpu().numpy()),
                                    steps,
                                )
                        ...
                        val_ctc_err = val_ctc_err_tot / (j + 1)
                        ...
                        sw.add_scalar("Validation/CTC_loss", val_ctc_err, steps)
