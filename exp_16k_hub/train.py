...
from exp_16k_hub.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_16k_hub.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, STFT_consistency_loss, MultiResolutionDiscriminator, t_axis_distill_loss
from exp_16k_hub.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def train(rank, world_size, h):
    ...
    mpd.train()
    mrd.train()
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("/mnt/nvme_share/srt30/hubert-base-ls960")
    hubert = HubertModel.from_pretrained("/mnt/nvme_share/srt30/hubert-base-ls960").eval().to(device)
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_g_np = y_g.squeeze(1).detach().cpu().numpy()
            #新增hubert语义损失
            with torch.no_grad():
                input_y = feature_extractor(list(y_np), sampling_rate=h.sampling_rate, return_tensors="pt").input_values.to(device)
                input_y_g = feature_extractor(list(y_g_np), sampling_rate=h.sampling_rate, return_tensors="pt").input_values.to(device)
            ...
            L_G = 45 * L_A + 100 * L_P + 20 * L_S + L_W + codebook_loss*10 + commitment_loss*2.5 + semantic_loss*5
            ...
            if rank ==0:
                ...
                if steps % h.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        semantic_err = semantic_loss.item()

                    print('Steps : {:d}, Gen Loss Total : {:4.3f}, Amplitude Loss : {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Phase Time Difference Loss : {:4.3f}, STFT Consistency Loss : {:4.3f}, Real Part Loss : {:4.3f}, Imaginary Part Loss : {:4.3f}, Mel Spectrogram Loss : {:4.3f}, Mel Spectrogram L2 Loss : {:4.3f}, Commit Loss : {:4.3f}, Semantic Loss : {:4.3f}, s/b : {:4.3f}'.
                        format(steps, L_G, A_error, IP_error, GD_error, PTD_error, C_error, R_error, I_error, Mel_error, Mel_L2_error, commit_loss, semantic_err, time.time() - start))
...
def main():
    ...
    config_file = '/mnt/nvme_share/srt30/APCodec-AP-BWE-Reproduction/exp_16k_hub/config.json'
