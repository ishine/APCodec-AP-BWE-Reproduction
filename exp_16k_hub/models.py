...
from exp_16k_hub.utils import init_weights, get_padding
import numpy as np
from exp_16k_hub.quantize import ResidualFSQ
...
def t_axis_distill_loss(feature, target_feature, lambda_sim=1.0):
    n = min(feature.size(1), target_feature.size(1))  # 对齐长度
    l1 = F.l1_loss(feature[:, :n], target_feature[:, :n], reduction='mean')
    cos = F.cosine_similarity(feature[:, :n], target_feature[:, :n], dim=-1).mean()
