...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.split:
            ...
            if audio_lr.size(1)*6 != original_length:
                if audio_lr.size(1)*6 > original_length:
                    audio_hr = torch.nn.functional.pad(audio_hr, (0, audio_lr.size(1)*6 - original_length), 'constant')
                else:
                    audio_hr = audio_hr[:, :audio_lr.size(1)*6]

        if torch.isnan(audio_hr).any():
            print(" NaN detected in audio_hr")

        if torch.isnan(audio_lr).any():
            print(" NaN detected in audio_lr")
