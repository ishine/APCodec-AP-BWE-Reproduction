...
from exp_0denoise.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_0denoise.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, STFT_consistency_loss, MultiResolutionDiscriminator
from exp_0denoise.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def train(h):
    ...
    training_filelist, validation_filelist, mixed_training_filelist, mixed_validation_filelist = get_dataset_filelist(h.input_training_wav_list, h.input_validation_wav_list,
                                                                                                                      h.input_training_mixed_wav_list, h.input_validation_mixed_wav_list)

    trainset = Dataset(training_filelist, mixed_training_filelist, h.segment_size, h.n_fft, h.num_mels_for_loss,
    ...
    validset = Dataset(validation_filelist, mixed_validation_filelist, h.segment_size, h.n_fft, h.num_mels_for_loss,
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            start_b = time.time()
            logamp, pha, rea, imag, y, y_mel = batch #y is clean audio
            ...
            y_g_mel = mel_spectrogram(y_g.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate, h.hop_size, h.win_size, 0, None)
            optim_d.zero_grad()
...
def main():
    ...
    config_file = '/mnt/nvme_share/srt30/APCodec-AP-BWE-Reproduction/exp_0denoise/config.json'
