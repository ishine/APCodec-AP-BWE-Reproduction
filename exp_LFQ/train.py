...
from exp_LFQ.dataset import Dataset, amp_pha_specturm, get_dataset_filelist, mel_spectrogram
from exp_LFQ.models import (
    Decoder,
    Encoder,
    ...
    try:
        ...
        for epoch in range(max(0, last_epoch), h.training_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                latent, commitment_loss = encoder(logamp_nb, pha_nb)
                ...
                if steps % h.summary_interval == 0 and rank == 0:
                    ...
                    sw.add_scalar("Training/Mel_Spectrogram_L2_Loss", Mel_L2_error, steps)
                    sw.add_scalar("Training/Discriminator_Loss", L_D, steps)
                    sw.add_scalar("Training/Amplitude_Loss", A_error, steps)
                    sw.add_scalar("Training/Instantaneous_Phase_Loss", IP_error, steps)
                    sw.add_scalar("Training/Group_Delay_Loss", GD_error, steps)
                    sw.add_scalar("Training/Phase_Time_Difference_Loss", PTD_error, steps)
                    sw.add_scalar("Training/STFT_Consistency_Loss", C_error, steps)
                    sw.add_scalar("Training/Real_Part_Loss", R_error, steps)
                    sw.add_scalar("Training/Imaginary_Part_Loss", I_error, steps)
                    sw.add_scalar("Training/Commitment_Loss", commit_loss, steps)
                ...
                if steps % h.validation_interval == 0 and rank == 0:  # 只在主进程进行验证
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            latent, _ = encoder(logamp_nb.to(device), pha_nb.to(device))
                            logamp_wb_g, pha_wb_g, rea_wb_g, imag_wb_g, y_wb_g = decoder(latent)
...
def main():
    ...
    config_file = "/mnt/nvme_share/srt30/APCodec-AP-BWE-Reproduction/exp_LFQ/config.json"
