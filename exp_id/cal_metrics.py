...
def cal_spk_acc(prediction_file):
    df = pd.read_csv(prediction_file, names=['filename','true_spk', 'predict_spk'])
    correct = (df['true_spk'] == df['predict_spk']).sum()
    total = len(df)
...
def main(h):
    ...
    spk_acc = cal_spk_acc(h.prediction_file)
    ...
    print('Speaker Accuracy: {:.3f}'.format(spk_acc))

if __name__ == '__main__':
    parser = argparse.ArgumentParser()

    parser.add_argument('--reference_wav_dir', default='/mnt/nvme_share/srt30/Datasets/VCTK-0.92/wav48/speaker/test')
    parser.add_argument('--synthesis_wav_dir', default='/mnt/nvme_share/srt30/checkpoint/exp_id/output_wav_140k')
    parser.add_argument('--prediction_file', default='/mnt/nvme_share/srt30/checkpoint/exp_id/output_speaker_140k/speaker_predictions.csv')
