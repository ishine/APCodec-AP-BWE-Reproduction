...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, n_fft, num_mels_for_loss,
        ...
        self.spk2id = {spk: i for i, spk in enumerate(all_speakers)}

    def __getitem__(self, index):
        filename = self.audio_files[index]
        basename = os.path.splitext(os.path.basename(filename))[0]
        speaker_str = basename.split("_")[0]
        speaker = self.spk2id[speaker_str]
        ...
        return (log_amplitude.squeeze(), phase.squeeze(), rea.squeeze(), imag.squeeze(), audio.squeeze(0), mel_loss.squeeze(), speaker)
