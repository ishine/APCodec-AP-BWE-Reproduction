...
from exp_fsq.utils import init_weights, get_padding
import numpy as np
from exp_fsq.quantize import ResidualFSQ
...
class SpeakerHead(nn.Module):
    def __init__(self, input_dim, num_speakers, hidden_dim=256):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),#[B,hidden_dim]
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, num_speakers)#[B,num_speakers]
        )
    def forward(self,x):
        return self.net(x)
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.speaker_head = SpeakerHead(input_dim=h.latent_dim, num_speakers=109, hidden_dim=256)
    ...
    def forward(self, logamp, pha, speakers=None):
        ...
        latent_pooled = lbq.mean(dim=2) #[B,C]
        logits = self.speaker_head(latent_pooled)#[B,num_speakers]
        spk_loss = torch.tensor(0.0, device=latent.device)
        if speakers is not None:
            spk_loss = F.cross_entropy(logits, speakers)

        return latent,codes,commitment_loss,codebook_loss,logits,spk_loss
