...
class ResidualFSQ(nn.Module):
    ...
    def forward(self, z, n_quantizers: int = None):
        ...
        if self.training and self.quantizer_dropout > 0:
            dropout_mask = torch.rand(z.shape[0], device=z.device) < self.quantizer_dropout
            possible_quantizers = torch.tensor([2, 4, 8], device=z.device)
            random_choices = possible_quantizers[torch.randint(0, len(possible_quantizers), (z.shape[0],), device=z.device)]
            n_quantizers_tensor[dropout_mask] = random_choices[dropout_mask]
...
if __name__ == "__main__":
    model = ResidualFSQ(input_dim=512, n_quantizers=4, codebook_size=1024, codebook_dim=32)
    x = torch.randn(8, 512, 80)
    y = model(x)