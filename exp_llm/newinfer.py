...
def generate_tokens(h,filename,encoder,tokenizer,device):
...
def main():
    ...
    for filename in tqdm(filelist, desc="inference", unit="file"):
        try:
            tokenset = generate_tokens(h, filename, encoder, tokenizer, device)
            file_segments = prediction(tokenset, tokenizer, instruction_prompt, model)
            segments = file_segments[os.path.basename(filename)]
            total_token = []
            for tokens in segments:
                total_token.extend(tokens)
            repaired_audio = repair_audio(total_token, encoder, decoder, device)
            output_path = os.path.join(h.test_wav_output_dir, f"{os.path.basename(filename)}")
            sf.write(output_path, repaired_audio, h.sampling_rate)
            logging.info(f"Repaired audio saved to {output_path}")
