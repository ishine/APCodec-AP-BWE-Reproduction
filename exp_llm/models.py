...
from exp_llm.utils import init_weights, get_padding
import numpy as np
from exp_llm.quantize import ResidualFSQ
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, logamp, pha):
        ...
        #drop tokens
        batch_size, channels, frames = latent.shape
        origin_len = frames
        _, n_quantizers, _ = codes.shape
        drop_indices = torch.arange(frames)[4::5].to(latent.device)
        mask_latent = torch.ones(batch_size, channels, frames, dtype=torch.bool, device=latent.device)
        mask_codes = torch.ones(batch_size, n_quantizers, frames, dtype=torch.bool, device=latent.device)
        mask_latent[:,:,drop_indices] = False
        mask_codes[:,:,drop_indices] = False
        latent = latent[mask_latent].view(batch_size, channels, -1)
        codes = codes[mask_codes].view(batch_size, n_quantizers, -1)

        return latent,codes,commitment_loss,codebook_loss,origin_len,gt_latent

class TokenRestore(nn.Module):
    def __init__(self, input_dim=32, num_layers=1, num_heads=2, ff_dim=128, dropout=0.1, max_len=2500):
        super().__init__()
...
class Decoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.adanorm_num_embeddings=None
        self.intermediate_dim=512
        self.token_restore = TokenRestore(input_dim=h.latent_dim)
    ...
    def forward(self, latent, origin_len):

        latent, kept_mask = token_pad(latent, origin_len)
        restore_latent = self.token_restore(latent)
        latent = restore_latent
        ...
        return logamp, pha, rea, imag, audio.unsqueeze(1), restore_latent, kept_mask
...
def token_pad(latent, origin_len):
    
    batch_size, channels, frames_short = latent.shape
    origin_group = 5
    current_group = 4
    num_groups = origin_len // origin_group
    restore_latent = torch.zeros(batch_size, channels, origin_len, device=latent.device)
    kept_indices = []
    for g in range(num_groups):
        base = g * origin_group
        kept_indices.extend([base+i for i in range(current_group)])
    if num_groups*origin_group < origin_len:
        for i in range(num_groups*origin_group, origin_len):
            kept_indices.append(i)
    kept_indices = torch.tensor(kept_indices, device=latent.device)
    restore_latent[:, :, kept_indices] = latent
    kept_mask = torch.zeros(origin_len, dtype=torch.bool, device=latent.device)
    kept_mask[kept_indices] = True

    return restore_latent, kept_mask
