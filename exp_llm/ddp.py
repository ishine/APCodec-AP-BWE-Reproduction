...
from exp_llm.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_llm.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, STFT_consistency_loss, MultiResolutionDiscriminator
from exp_llm.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def train(rank, world_size, h):
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            latent,codes,commitment_loss,codebook_loss,origin_len,gt_latent = encoder(logamp, pha)
            logamp_g, pha_g, rea_g, imag_g, y_g, restore_latent, kept_mask = decoder(latent, origin_len)
            ...
            restore_loss = F.l1_loss(restore_latent[:, :, ~kept_mask], gt_latent[:, :, ~kept_mask])
            # Losses defined on final waveforms
            L_W = L_GAN_G + L_FM + 45 * L_Mel + 45 * L_Mel_L2 

            L_G = 45 * L_A + 100 * L_P + 20 * L_S + L_W + codebook_loss*10 +commitment_loss*2.5 + 20 * restore_loss
            ...
            if rank == 0:
                ...
                if steps % h.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        restore_error = restore_loss.item()

                    print('Steps : {:d}, Gen Loss Total : {:4.3f}, Amplitude Loss : {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Phase Time Difference Loss : {:4.3f}, STFT Consistency Loss : {:4.3f}, Real Part Loss : {:4.3f}, Imaginary Part Loss : {:4.3f}, Mel Spectrogram Loss : {:4.3f}, Mel Spectrogram L2 Loss : {:4.3f}, Commit Loss : {:4.3f}, Restore Loss : {:4.3f}, s/b : {:4.3f}'.
                        format(steps, L_G, A_error, IP_error, GD_error, PTD_error, C_error, R_error, I_error, Mel_error, Mel_L2_error, commit_loss, restore_error, time.time() - start))

                # checkpointing
                if steps % h.checkpoint_interval == 0 and steps != 0:
                    if val_A_err < best_val_loss:
                        best_val_loss = val_A_err
                        print(f"[Checkpoint] Saving best model at step {steps}, val_A_err = {val_A_err:.4f}")
                ...
                if steps % h.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            logamp, pha, rea, imag, y, y_mel = batch
                            latent,_,_,_,origin_len,gt_latent = encoder(logamp.to(device), pha.to(device))
                            logamp_g, pha_g, rea_g, imag_g, y_g, _, _ = decoder(latent, origin_len)
