...
from exp_nasr.utils import init_weights, get_padding
import numpy as np
from exp_nasr.quantize import ResidualFSQ
...
class CTCHead(nn.Module):
    def __init__(self, input_dim, vocab_size, hidden_dim=512, dropout_rate=0.1):
        super().__init__()
        self.conv1d = nn.Conv1d(input_dim, input_dim, kernel_size=7, padding=3, groups=1)
        self.conv_ln = nn.LayerNorm(input_dim)
    ...
    def forward(self, x):  # x: [B, C, T]
        ...
        conv_input = x.permute(0, 2, 1) # [B, C, T]
        conv_out = self.conv1d(conv_input) # [B, C, T]
        conv_out = conv_out.permute(0, 2, 1) # [B, T, C]
        x = self.conv_ln(x + conv_out) # 残差连接 + 归一化
        x = self.mlp(x)
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        # 添加 CTC 头
        self.ctc_head = CTCHead(input_dim=h.latent_dim, vocab_size=28)  # 假设词汇表大小为28
