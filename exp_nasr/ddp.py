...
from exp_nasr.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_nasr.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, STFT_consistency_loss, MultiResolutionDiscriminator
from exp_nasr.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def collate_fn(batch):
    audio, text_label, target_length, input_length = zip(*batch)
    ...
    return audio_padded, text_labels, target_lengths, input_lengths
...
def train(rank, world_size, h):
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            audio, text_labels, target_lengths, input_lengths = batch
            ...
            latent, _, commitment_loss, codebook_loss, ctc_loss,_ = encoder(logamp, pha, text_labels, target_lengths, input_lengths)
            ...
            if rank == 0:
                ...
                if steps % h.stdout_interval == 0:
                    with torch.no_grad():
                        ...
                        print(
                            "Steps : {:d}, Gen Loss Total : {:4.3f}, Amplitude Loss : {:4.3f}, Instantaneous Phase Loss : {:4.3f}, Group Delay Loss : {:4.3f}, Phase Time Difference Loss : {:4.3f}, STFT Consistency Loss : {:4.3f}, Real Part Loss : {:4.3f}, Imaginary Part Loss : {:4.3f}, Mel Spectrogram Loss : {:4.3f}, Mel Spectrogram L2 Loss : {:4.3f}, Commit Loss : {:4.3f}, CTC Loss : {:4.3f}, s/b : {:4.3f}".format(
                                steps, L_G, A_error, IP_error, GD_error, PTD_error, C_error, R_error, I_error,
                                Mel_error, Mel_L2_error, commit_loss, ctc_loss, time.time() - start
                            )
                        )
                ...
                if steps % h.validation_interval == 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            input_lengths = input_lengths.to(device, non_blocking=True)
                            logamp, pha, rea, imag = amp_pha_specturm(audio.squeeze(1), h.n_fft, h.hop_size, h.win_size)
                            latent,_, _, _, ctc_loss,_ = encoder.module(logamp.to(device), pha.to(device), text_labels.to(device), target_lengths.to(device), input_lengths.to(device))
