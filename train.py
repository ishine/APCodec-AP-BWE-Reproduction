...
def train(rank, world_size, h):
    """Training function for distributed multi-GPU setup."""
    ...
    sw = SummaryWriter(os.path.join(h.checkpoint_path, "logs")) if rank == 0 else None
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if steps % h.stdout_interval == 0 and rank == 0:
                ...
                print(
                    f"Steps : {steps}, Gen Loss Total : {L_G:4.3f}, Amplitude Loss : {A_error:4.3f}, "
                    f"Instantaneous Phase Loss : {IP_error:4.3f}, Group Delay Loss : {GD_error:4.3f}, "
                    f"Phase Time Difference Loss : {PTD_error:4.3f}, STFT Consistency Loss : {C_error:4.3f}, "
                    f"Real Part Loss : {R_error:4.3f}, Imaginary Part Loss : {I_error:4.3f}, "
                    f"Mel Spectrogram Loss : {Mel_error:4.3f}, Mel Spectrogram L2 Loss : {Mel_L2_error:4.3f}, "
                    f"Commit Loss : {commit_loss:4.3f}, s/b : {time.time() - start_b:4.3f}"
                )

            # Checkpointing (only on rank 0)
            if steps % h.checkpoint_interval == 0 and steps != 0 and rank == 0:
                checkpoint_path = f"{h.checkpoint_path}/encoder_{steps:08d}"
                save_checkpoint(checkpoint_path, {"encoder": encoder.module.state_dict()})
                checkpoint_path = f"{h.checkpoint_path}/decoder_{steps:08d}"
            ...
            if steps % h.validation_interval == 0 and rank == 0:
                encoder.eval()
                decoder.eval()
                ...
                with torch.no_grad():
                    for j, batch in enumerate(validation_loader):
                        ...
                        if j <= 4:
                            ...
                            sw.add_figure(
                                f"generated/y_g_logamp_{j}",
                                plot_spectrogram(logamp_g[0].cpu().numpy()),
                                steps,
                            )
                            sw.add_figure(
                                f"generated/y_g_pha_{j}",
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        raise RuntimeError("CUDA is not available, multi-GPU training is not supported.")

    mp.spawn(train, args=(args.world_size, h), nprocs=args.world_size, join=True)
