...
def inference(h):
    ...
    os.makedirs(h.base_test_wav_output_dir, exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for i, filename in enumerate(filelist):
            ...
            base_latent, latent,codes,_,_ = encoder(logamp, pha)
            _, _, _, _, y_g = decoder(latent)
            _, _, _, _, base_y_g = decoder(base_latent)
            ...
            base_audio = base_y_g.squeeze()
            base_audio = base_audio.cpu().numpy()

            sf.write(os.path.join(h.test_wav_output_dir, filename.split('.')[0]+'.wav'), audio, h.sampling_rate,'PCM_16')
            sf.write(os.path.join(h.base_test_wav_output_dir, filename.split('.')[0]+'.wav'), base_audio, h.sampling_rate,'PCM_16')
