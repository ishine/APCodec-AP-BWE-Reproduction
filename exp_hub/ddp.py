...
from exp_hub.dataset import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
from exp_hub.models import Encoder, Decoder, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, amplitude_loss, phase_loss, STFT_consistency_loss, MultiResolutionDiscriminator
from exp_hub.utils import AttrDict, build_env, plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
...
def train(rank, world_size, h):
    ...
    for epoch in range(max(0, last_epoch), h.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            #新增hubert语义损失
            with torch.no_grad():
                y_lr = torchaudio.functional.resample(y.squeeze(1), orig_freq=48000, new_freq=16000)
                y_g_lr = torchaudio.functional.resample(y_g.squeeze(1), orig_freq=48000, new_freq=16000)
                hubert_y = hubert(y_lr)[0]
                hubert_y_g = hubert(y_g_lr)[0]
            
            semantic_loss = F.l1_loss(hubert_y, hubert_y_g)

            # Losses defined on final waveforms
            L_W = L_GAN_G + L_FM + 45 * L_Mel + 45 * L_Mel_L2 

            L_G = 45 * L_A + 100 * L_P + 20 * L_S + L_W + codebook_loss*10 +commitment_loss*2.5 + 10*semantic_loss
...
def main():
    ...
    config_file = '/mnt/nvme_share/srt30/APCodec-AP-BWE-Reproduction/exp_hub/config.json'
