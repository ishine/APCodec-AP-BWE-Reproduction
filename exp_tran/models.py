...
class ASRDecoder(nn.Module):
    def __init__(self, vocab_size=30, input_dim=32, hidden_dim=128, nhead=2, dim_ff=128, num_layers=3, dropout=0.1):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, hidden_dim)
        self.embedding = nn.Embedding(vocab_size, hidden_dim)
        decoder_layer = nn.TransformerDecoderLayer(d_model=hidden_dim, nhead=nhead, dim_feedforward=dim_ff, dropout=dropout)
        self.decoder = nn.TransformerDecoder(decoder_layer, num_layers=num_layers)
        self.output_proj = nn.Linear(hidden_dim, vocab_size)

    def forward(self, tgt, memory):
        """
        tgt: token 序列 [text_len, B]
        memory: 编码器输出 [frames, B, input_dim]
        """
        text_len = tgt.size(0)
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(text_len).to(tgt.device)
        tgt_key_padding_mask = (tgt == 0).transpose(0,1).bool()
        memory = self.input_proj(memory) #[frames, B, latent_dim] -> [frames, B, hidden_dim]
        tgt_embed = self.embedding(tgt)  # [text_len, B, hidden_dim]
        output= self.decoder(tgt_embed, memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask)  # [text_len, B, hidden_dim]
        logits = self.output_proj(output)  # [text_len, B, vocab_size]
        return logits
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.asrLoss = nn.CrossEntropyLoss(ignore_index=0)
    ...
    def forward(self, logamp, pha, text_labels=None):
        ...
        if text_labels is not None:
            memory = latent_before_quantizer.permute(2, 0, 1)#[frames, B, latent_dim]
            tgt_input = text_labels[:, :-1].transpose(0,1).contiguous()#[T_label-1, B]
            tgt_target = text_labels[:, 1:].transpose(0,1).contiguous()#[T_label-1, B]
            logits = self.asr_decoder(tgt_input, memory)#[T_label-1, B, vocab_size]
            asr_loss = self.asrLoss(logits.reshape(-1, logits.shape[-1]), tgt_target.reshape(-1))

        return latent_before_quantizer,latent,codes,commitment_loss,codebook_loss,asr_loss
...
def STFT_consistency_loss(rea_r, rea_g, imag_r, imag_g):
    ...
    min_T_rea = min(rea_r.shape[-1], rea_g.shape[-1])
    rea_r = rea_r[..., :min_T_rea]
    rea_g= rea_g[..., :min_T_rea]

    min_T_imag = min(imag_r.shape[-1], imag_g.shape[-1])
    imag_r = imag_r[..., :min_T_imag]
    imag_g= imag_g[..., :min_T_imag]
