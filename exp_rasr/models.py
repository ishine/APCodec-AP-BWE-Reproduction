...
import numpy as np
from exp_rasr.quantize import ResidualFSQ
...
class RNNDecoder(nn.Module):
    def __init__(self, vocab_size=30, input_dim=256, hidden_dim=512, num_layers=2, dropout=0.2):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, input_dim)
        self.rnn = nn.GRU(input_dim, hidden_dim, num_layers, dropout=dropout)
    ...
    def forward(self, tgt, memory):
        """
        tgt: token 序列 [T, B]
        memory: 编码器输出 [T_src, B, hidden_dim]
        """
        embedded = self.embedding(tgt)  # [T, B, input_dim]
        outputs, hidden = self.rnn(embedded)  # [T, B, hidden_dim]

        attn_output, _ = self.attn(outputs, memory, memory)  # [T, B, hidden_dim]
        logits = self.output_proj(attn_output)  # [T, B, vocab_size]
        return logits
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        # 添加 ASR
        self.asr_decoder = RNNDecoder(vocab_size=30, hidden_dim=512)
        self.asr_proj_linear = nn.Linear(h.latent_dim, 512)
    ...
    def forward(self, logamp, pha, text_labels=None):
        ...
        if text_labels is not None:
            memory = latent_before_quantizer.permute(2, 0, 1)#[T_frame, B, latent_dim]
            memory = self.asr_proj_linear(memory)#[T_frame, B, 512]
